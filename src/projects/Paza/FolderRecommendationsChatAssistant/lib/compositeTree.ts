...
export function compositeTree(
  ...
  for (const p of proposals) {
    ...
    const siblings = isRootAdd ? composited : (parent!.children = parent!.children ?? []);
    const insertIndex = p.insertAfterNodeId
      ? siblings.findIndex((node) => node.id === p.insertAfterNodeId)
      : -1;
    if (insertIndex >= 0) {
      siblings.splice(insertIndex + 1, 0, synthetic);
    } else {
      siblings.push(synthetic);
    }
  }
