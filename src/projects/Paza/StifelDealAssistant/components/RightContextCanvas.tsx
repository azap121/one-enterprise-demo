...
type BaseRightCanvasTab = 'validation-plan' | 'enhanced-index' | 'filing-review' | 'files' | 'qa' | 'skills';
...
export function getComposerPlaceholderForRightTab(tab: RightCanvasTab | null) {
  if (tab === 'skills') {
    return 'Describe a reusable skill, or upload the framework or template it should follow...';
  }
  return undefined;
}
        ...
        >
          ...
          >
            ...
            <MenuItem onClick={() => addContextTab('skills')}>
              ...
              <ListItemText
                primary="Skills"
                secondary="Reusable workflows grounded in your templates"
                primaryTypographyProps={{ fontSize: 13 }}
                secondaryTypographyProps={{ fontSize: 12 }}
              />
            </MenuItem>
...
export function ContextTabPill({
  tab,
  selected,
  onSelect,
  onClose,
...
export function RightCanvasTabContent({
  activeTab,
  state,
  dispatch,
  notesByRowId,
...
// One concept, per the 2026-07-16 session: a skill = the reusable instructions PLUS the
// framework/template it follows. Upload the artifact, get a skill; describe the workflow,
// get a skill. No separate Templates surface.
const SKILL_LIBRARY = [
  {
    name: 'Churn / NRR buyer response',
    grounding: 'ARR / churn analysis checklist',
    status: 'Firm-shared',
  },
  {
    name: 'Source-code disclosure review',
    grounding: 'Round 1 disclosure rules',
    status: 'Firm-shared',
  },
  {
    name: 'DPA / SOC 2 evidence search',
    grounding: 'Q&A response template',
    status: 'Draft',
  },
  {
    name: 'Senior banker update',
    grounding: 'Banker update format',
    status: 'Firm-shared',
  },
] as const;

function SkillsContextView({ bottomInset }: { bottomInset: number }) {
  const [pendingUpload, setPendingUpload] = useState<'idle' | 'processing' | 'ready'>('idle');

  useEffect(() => {
    if (pendingUpload !== 'processing') return undefined;
    const timer = window.setTimeout(() => setPendingUpload('ready'), 1600);
  ...
  return (
    <ContextWorkspaceShell
      title="Skills"
      subtitle="A skill is a reusable workflow plus the firm framework or template it follows — describe it once, or upload the artifact your team already uses."
      bottomInset={bottomInset}
    >
      <Stack
        spacing={1.25}
      ...
      >
        ...
        <Typography sx={{ fontSize: 14, fontWeight: 600, color: 'text.primary' }}>
          Upload a framework or template
        </Typography>
        <Typography sx={{ fontSize: 12.5, lineHeight: 1.55, color: 'text.secondary' }}>
          Drop a DD request list, Excel modelling framework, response template, or playbook.
          Datasite AI turns it into a skill with full room context — cited outputs follow your
          structure, wording, and approval path, and the skill can be shared firm-wide.
        </Typography>
        <HaloButton
          size="small"
          variant="outlined"
          startIcon={<FontAwesomeIcon icon={faBookOpenLines} />}
          disabled={pendingUpload === 'processing'}
          onClick={() => setPendingUpload('processing')}
          sx={{ textTransform: 'none' }}
        >
          {pendingUpload === 'processing' ? 'Reading framework...' : 'Upload framework or template'}
        </HaloButton>
        {pendingUpload !== 'idle' ? (
          <Typography sx={{ fontSize: 12.5, color: pendingUpload === 'ready' ? 'text.primary' : 'text.secondary' }}>
            {pendingUpload === 'ready'
              ? 'Tech M&A DD request list v4.xlsx — draft skill ready: "Check the room against our request list". Review and approve before first run.'
              : 'Tech M&A DD request list v4.xlsx — classifying against Project Aldgate sources...'}
          </Typography>
        ) : null}
      </Stack>
      ...
      <ContextWorkspaceSection
        ...
        body="Ask for a reusable workflow — Datasite AI drafts the objective, inputs, permission checks, output format, and approval checkpoints. Refine it, then share it so the whole team runs the same play."
      />
    </ContextWorkspaceShell>
  );
}
