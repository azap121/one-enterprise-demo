        ...
        <Box sx={{ flex: 1, minWidth: 0, height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          {showFullEmpty ? (
            <FullChatEmptyState
              ...
              onSelectRetroPrompt={() => dispatch({ type: 'SELECT_RETRO_FILING_PROMPT' })}
              onSelectClientDropPrompt={() => dispatch({ type: 'SELECT_CLIENT_DROP_PROMPT' })}
...
// Rendered at the top level (FolderRecommendationsChatAssistant) so both the rail's
// Search row and the top-centre ask-anything bar open the same centred dialog.
export function SearchSpotlightDialog({
  open,
  value,
  recentChats,
  onChange,
