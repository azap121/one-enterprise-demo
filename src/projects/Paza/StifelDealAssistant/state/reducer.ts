...
import { getFilingSpec, type FilingVariant } from './filingVariants';
...
// Scripted reply for the gap-finder prompt — the pivot from the refuted MD-readout brief.
// Defensive framing: "there is no perfect data room; the VDR can only decrease transaction
// value" — the agent's job is finding the gaps before a senior or a buyer does.
const GAP_FINDER_REPLY = `I checked the sandbox against your DD request list and the shape of comparable software sell-sides. Three gaps and one red flag to close before anyone senior walks the room:

PREDICTED GAPS — request-list items with no answering document
• 05 Legal — no signed DPAs for the top-20 customers. The DPA template is filed; signed copies are not.
• 03 Financials — FY2023 statutory accounts missing. Buyers will ask for the three-year set.
• 06 Technology — no disaster-recovery / business-continuity policy anywhere in the room.

RED FLAG — an inconsistency a buyer will find
• The management accounts (May-2026) and the trial balance (Jun-2026) are one period apart. If a bidder rebuilds the bridge, the numbers won't tie.

None of this is visible to bidders — it's your gap list, not theirs. Want me to draft the chase list for the client, or hold specific items back from the next disclosure wave?`;
...
function getScriptedReply(prompt: string): string | null {
  ...
  if (normalized.includes('where are the gaps')) return GAP_FINDER_REPLY;
  return null;
}
...
export function reducer(state: WorkspaceState, action: WorkspaceAction): WorkspaceState {
  switch (action.type) {
    ...
    case 'SELECT_RETRO_FILING_PROMPT':
    case 'SELECT_CLIENT_DROP_PROMPT': {
      const variant: FilingVariant =
      ...
      track('stifel_deal_assistant.prompt.select', {
        prompt: variant === 'uploads' ? 'file-latest-upload' : variant === 'retro' ? 'retro-filing' : 'client-drop',
      });
      return {
        ...state,
        ...
        recommendationStepIndex: 0,
        rationaleExpanded: false,
        proposals: spec.scenario.proposals,
        tree: spec.scenario.nodes,
        localProposals: [],
        overrides: {},
        collapsedNodeIds: [],
        editingNodeId: null,
        ...
        messages: [
          {
            id: `user-filing-prompt-${variant}`,
            role: 'user',
            kind: 'text',
            content: spec.copy.userPrompt,
          },
          {
            id: `assistant-filing-thinking-${variant}`,
            role: 'assistant',
            kind: 'thinking',
            content: spec.copy.formingMessage,
          },
        ],
      };
    }
    ...
    case 'RECOMMENDATION_READY': {
      ...
      if (state.flow === 'filing') {
        const spec = getFilingSpec(state.filingVariant);
        track('stifel_deal_assistant.filing_plan.ready', { variant: state.filingVariant });
        return {
          ...state,
          stage: 'proposal-ready',
          assistantMode: 'full',
          recommendationStepIndex: spec.steps.length,
          rationaleExpanded: false,
          messages: [
            ...state.messages,
            {
              ...
              content: spec.copy.proposalSummary,
            },
          ],
        };
      }
      ...
      return {
        ...
        messages: [
          ...state.messages,
          {
            ...
            content:
              state.flow === 'filing'
                ? getFilingSpec(state.filingVariant).copy.savingMessage
    ...
    case 'SAVE_STEP_DONE': {
      const saveCap = state.flow === 'filing' ? getFilingSpec(state.filingVariant).saveSteps.length : saveSteps.length;
      return {
        ...state,
    ...
    case 'INDEX_SAVED':
      ...
      return {
        ...
        stage: 'saved',
        assistantMode: 'full',
        saveStepIndex: state.flow === 'filing' ? getFilingSpec(state.filingVariant).saveSteps.length : saveSteps.length,
        dirty: false,
        ...
        messages: [
          ...state.messages,
          {
            ...
            role: 'assistant',
            kind: 'success',
            content: state.flow === 'filing' ? getFilingSpec(state.filingVariant).copy.savedBody : COPY.savedBody,
          },
        ],
      };
