...
import { CompactResultsPanel, TabbedDocViewer, InlineAiChatPanel, RESULTS_PANEL_WIDTH, type AiDocItem } from '~/projects/Graham/SearchSidecarB/components/AiSearchLayout';
...
const SearchSidecar: React.FC = () => {
  ...
  useEffect(() => {
    if (!isAiChat) { setAiOpenDocs([]); setAiActiveDocId(null); setAiReadIds(new Set()); }
  }, [isAiChat]);
    ...
    <DatasitePrototypeShell
      ...
      <Box
        ...
        <Box sx={{ flex: 1, display: 'flex', flexDirection: isAiChat ? 'row' : 'column', overflow: 'hidden', minWidth: 0, position: 'relative' }}>
          {isAiChat && (
            <>
              {/* Tabbed doc viewer — slides in from left when a doc is opened */}
              <Box sx={{
